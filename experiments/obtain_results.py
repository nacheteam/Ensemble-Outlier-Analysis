...
for dataset in datasets:
    ...
    plt.xlabel("Modelos", fontsize=5)
    plt.ylabel("Porcentaje de acierto", fontsize=5)
    plt.xticks(index, labels, fontsize=5, rotation=30)
    plt.title("Porcentaje de acierto en el conjunto " + dataset)
    plt.savefig("./imgs_exp1/accuracy/" + dataset + ".png")
    plt.close()
...
for dataset in datasets:
    ...
    plt.bar(index,times)
    plt.xlabel("Modelos", fontsize=5)
    plt.ylabel("Tiempo en segundos", fontsize=5)
    plt.xticks(index, labels, fontsize=5, rotation=30)
    plt.title("Tiempo de cómputo de los modelos en " + dataset)
    plt.savefig("./imgs_exp1/times/" + dataset + ".png")
    plt.close()
